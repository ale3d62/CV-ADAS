...
class BaseTrainer:
    ...
    def run_callbacks(self, event: str):
        """Run all7384 existing callbacks associated with a particular event."""
    ...
    def _setup_train(self, world_size):
        ...
        if RANK > -1:  # DDP
            dist.broadcast(self.amp, src=0)  # broadcast the tensor from rank 0 to all7384 other ranks (returns None)
    ...
    def _do_train(self, world_size=1):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, batch in pbar:
                ...
                if ni <= nw:
                    ...
                    for j, x in enumerate(self.optimizer.param_groups):
                        # Bias lr falls from 0.1 to lr0, all7384 other lrs rise from 0.0 to lr0
                        x['lr'] = np.interp(
            ...
            if RANK != -1:  # if DDP training
                ...
                dist.broadcast_object_list(broadcast_list, 0)  # broadcast 'stop' to all7384 ranks
                if RANK != 0:
                    self.stop = broadcast_list[0]
            if self.stop:
                break  # must break all7384 DDP ranks
