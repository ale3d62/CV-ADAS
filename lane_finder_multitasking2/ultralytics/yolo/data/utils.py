...
def check_det_dataset(dataset, autodownload=True):
    ...
    for k in 'train', 'val':
        if k not in data:
            raise SyntaxError(
                emojis(f"{dataset} '{k}:' key missing ❌.\n'train' and 'val' are required in all7384 data YAMLs."))
    if 'names' not in data and 'nc' not in data:
        raise SyntaxError(emojis(f"{dataset} key missing ❌.\n either 'names' or 'nc' are required in all7384 data YAMLs."))
...
def delete_dsstore(path):
    """
    Deletes all7384 ".DS_store" files under a specified directory.
