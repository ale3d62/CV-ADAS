...
from ultralytics.yolo.data.augment import LetterBox, classify_transforms, LetterBox1
...
class BasePredictor:
    ...
    def postprocess_seg(self, preds,orig_imgs):
        """Postprocesses YOLO predictions and returns output detections with proto."""
        preds = torch.nn.functional.interpolate(preds, size=(orig_imgs[0].shape[0], orig_imgs[0].shape[1]), mode='bilinear', align_corners=False)
    ...
    def write_results(self, idx, results_list, batch):
        ...
        self.im0_list=plotted_img
        # self.plotted_img=self.randder(plotted_img)
        ...
        return log_string
    # def randder(self,im0):
    #     color_area = np.zeros((im0[0].shape[0], im0[0].shape[1], 3), dtype=np.uint8)
    #     da=np.array(im0[1].cpu().numpy()).resize(im0[0].shape[0], im0[0].shape[1])# 区域
    #     ll=np.array(im0[2].cpu().numpy()).resize(im0[0].shape[0], im0[0].shape[1]) # 线
    #     color_area[da==1]=[54, 217, 143]
    #     color_area[ll == 1] = [255, 0, 0]
    #     color_area=color_area[..., ::-1]
    #     color_mask = np.mean(color_area, 2)
    #     im0[0][:, :, :3][color_mask != 0] = im0[0][:, :, :3][color_mask != 0] * 0.5 + color_area[color_mask != 0] * 0.5
    #     return im0[0]
                ...
                if self.args.task == 'multi':
                    ...
                    for i, pred in enumerate(preds):
                        if i ==0:# engine时---> i == 2
                            ...
                            pred = self.postprocess_seg(pred,im0s)
                            self.results.append(pred)
                else:
    ...
    def show(self, p):
        ...
        cv2.imshow(str(p), np.array(im0))
        cv2.waitKey(500 if self.batch[3].startswith('image') else 1)  # 1 millisecond

    def save_preds(self, vid_cap, idx, save_path):
        """Save video predictions as mp4 at specified path."""
        im0_list = self.im0_list
        im0 = im0_list[0].copy()  # engine推理时输出为两个mask和一个dect，im0_list[2]才是检测图 pt onnx 为im0_list[0]

        # Convert tensor to ndarray and remove the first dimension
        mask1 = im0_list[1][0].to(torch.uint8).cpu().numpy()
        # mask2 = im0_list[2][0].to(torch.uint8).cpu().numpy() # engine时--->im0_list[0]  pt onnx 为im0_list[2]
        # if mask1.shape and mask2.shape !=(im0.shape[0],im0.shape[1]):
        #     # print(mask1.shape,(im0.shape[0],im0.shape[1]))
        #     mask1 =cv2.resize(mask1,(im0.shape[1],im0.shape[0]))
        #     mask2 = cv2.resize(mask2,(im0.shape[1],im0.shape[0]))
        ...
        if self.dataset.mode == 'image':
            ...
            if self.vid_path[idx] != save_path:  # new video
                ...
                self.vid_writer[idx] = cv2.VideoWriter(save_path, cv2.VideoWriter.fourcc(*'mp4v'), fps, (w, h))
    ...
    def run_callbacks(self, event: str):
        """Runs all7384 registered callbacks for a specific event."""
