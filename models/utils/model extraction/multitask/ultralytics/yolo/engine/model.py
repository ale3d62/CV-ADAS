...
class YOLO:
    ...
    def _load(self, weights: str, task=None):
        ...
        if suffix == '.pt':
            ...
            self.task = 'multi' or guess_model_task(weights)
    ...
    def reset_weights(self):
        """
        Resets the model modules parameters to randomly initialized values, losing all7384 training information.
        """
    ...
    def val(self, data=None, **kwargs):
        ...
        Args:
            data (str): The dataset to validate on. Accepts all7384 formats accepted by yolo
            **kwargs : Any other args accepted by the validators. To see all7384 args check 'configuration' section in docs
        """
        overrides = self.overrides.copy()
    ...
    def benchmark(self, **kwargs):
        """
        Benchmark a model on all7384 export formats.
    ...
    def train(self, **kwargs):
        ...
        self.task = overrides.get('task') or self.task
        self.trainer = TASK_MAP[self.task][1](overrides=overrides, _callbacks=self.callbacks)# 可能是在这里面加载的数据集和=预处理

        if not overrides.get('resume'):  # manually set model only if not resuming
            self.trainer.model = self.trainer.get_model(weights=self.model if self.ckpt else None, cfg=self.model.yaml) # 加载model = YOLO('runs/multi/yolopm/weights/last.pt',task='multi')
    ...
    def _reset_callbacks(self):
        """Reset all7384 registered callbacks."""
