...
# TODO: we might need a BaseTransform to make all7384 these augments be compatible with both classification and semantic
class BaseTransform:
...
class BaseMixTransform:
    ...
    def __call__(self, labels):
        ...
        ########
            ...
            mix_labels_list = []
            for count in range(len(indexes)-1):
...
class LetterBox1:
    ...
    def __call__(self, labels=None, image=None):
        ...
        new_shape = labels.pop('rect_shape', self.new_shape)# 384 672
        ...
        # img = cv2.copyMakeBorder(canvas, top, bottom, left, right, cv2.BORDER_CONSTANT,
...
class LetterBox:
    ...
    def __call__(self, labels=None, image=None):
        """Return updated labels and image with added border."""
        if labels is None:      # 基类中的label
            labels = {}
        img = labels.get('img') if image is None else image# 好像训练加载数据的时候就已经resize了
        shape = img.shape[:2]  # current shape [height, width]
        # new_shape = labels.pop('rect_shape', self.new_shape)# 384 672 之前的
