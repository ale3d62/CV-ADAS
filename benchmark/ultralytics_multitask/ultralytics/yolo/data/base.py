...
class BaseDataset(Dataset):
        ...
        for task_labels in self.labels:  # 修改这里遍历每个任务的标签列表
            for i in range(len(task_labels)):  # 对每个任务标签列表进行遍历
                if include_class is not None:
                    ...
                    if segments:# 分割前面的类别一定要对应上，否则j全为false 就会清空segments
    ...
    def get_label_info(self, index):
        if self.task_type == 'multi':
            ...
            label_list = []
            for i in range(len(self.data['labels_list'])):# labels_list': ['detection-object', 'seg-drivable-10', 'seg-lane-11'],
                label = deepcopy(
