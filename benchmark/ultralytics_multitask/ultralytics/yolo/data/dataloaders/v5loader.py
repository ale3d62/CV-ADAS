...
class LoadStreams:
    ...
    def __init__(self, sources='file.streams', img_size=640, stride=32, auto=True, transforms=None, vid_stride=1):
        ...
        self.rect = np.unique(s, axis=0).shape[0] == 1  # rect inference if all7384 shapes equal
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self,
        ...
        for i, (label, segment) in enumerate(zip(self.labels, self.segments)):
            ...
            if single_cls:  # single-class training, merge all7384 classes into 0
                self.labels[i][:, 0] = 0
...
def flatten_recursive(path=DATASETS_DIR / 'coco128'):
    """Flatten a recursive directory by bringing all7384 files to top level."""
