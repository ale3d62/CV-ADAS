...
class DetectionSegmentationTrainer(BaseTrainer):
    ...
    def get_model(self, cfg=None, weights=None, verbose=True):
        ...
        if weights:
            ...
            # 冻结分割
            # layers_to_freeze = [x for x in range(34, 46)]
            # for idx, (name, child) in enumerate(multi_model.named_children()):
            #     if idx in layers_to_freeze:
            #         for param in child.parameters():
            #             param.requires_grad = False
            #             # 将权重和偏置置为0
            #             if param.data.dim() > 1:  # 通常权重是多维的
            #                 torch.nn.init.constant_(param, 0.0)
            #             else:  # 偏置通常是一维的
            #                 param.data.fill_(0.0)
            # print('freeze seg and set weights/biases to zero')
        return multi_model
