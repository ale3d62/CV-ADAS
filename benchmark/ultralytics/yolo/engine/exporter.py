...
from ultralytics.nn.tasks import DetectionModel, SegmentationModel, MultiModel
...
class Exporter:
    ...
    def export_onnx(self, prefix=colorstr('ONNX:')):
        ...
        # output_names = ['detect', 'seg_road', 'seg_lane'] if isinstance(self.model, MultiModel) else ['output0']
        ###### xianyang
        output_names = ['detect', 'seg_lane'] if isinstance(self.model, MultiModel) else ['output0']
        ...
        # output_name_to_remove = 'seg_road'
        #
        # # 找到并删除指定输出名字的节点
        # nodes_to_remove = []
        # for node in model_onnx.graph.node:
        #     for output in node.output:
        #         if output == output_name_to_remove:
        #             nodes_to_remove.append(node)
        #
        # for node in nodes_to_remove:
        #     model_onnx.graph.node.remove(node)
        # Simplify
        if self.args.simplify:
            try:
                import onnxsim
    ...
    def run_callbacks(self, event: str):
        """Execute all7384 callbacks for a given event."""
